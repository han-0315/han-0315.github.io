...
이번 주차에는 **오토스케일링**에 대해 진행했다. 순서는 파드의 오토스케일링을 살펴본 뒤 클러스터(노드) 스케일링 방식을 살펴본다.

**파드 기반으로 오토스케일링**으로 HPA, VPA 그리고 노드에 비례해서 파드 개수를 조정하는 CPA까지 있다. **노드 기반으로 오토스케일링**으로 KEDA, CA, Karpenter 까지 진행하면서 이번주차는 끝난다.

**파드 기반 스케일링**은 파드의 리소스 사용량을 기준으로 진행한다. 파드의 개수를 늘리는 것은 부하분산이 없다면 의미없는 일이지만, `selector` 를 통한 부하분산을 지원하기에 효과적이다. 
또한, VPA는 리소스를 늘리나 재실행이 필요하다는 단점이 있다. [관련 내용이 베타 업데이트되어 앞으로 재실행없이 리소스 변경이 가능할지도 모른다. ([Docs](https://kubernetes.io/blog/2023/05/12/in-place-pod-resize-alpha/))], HPA과 VPA 모두 쿠버네티스에서 지원해줘, 플러그인 설치 없이 진행가능하다. 

**노드 기반 스케일링은** CA, KEDA, Karpenter가 있다. CA는 파드를 통해 모니터링하고, 리소스 메트릭을 통해 스케일링한다. KEDA는 CA와는 다르게 리소스 메트릭이 아닌 **이벤트 기반**으로 스케일여부를 결정한다. 마지막으로 Karpenter는 중간에 ASG를 거치지 않고 바로 인스턴스를 제공하여 초 단위로 스케일링 가능하다는 장점이 있다.

**주제가 오토스케일링이다보니 끝나고 나서 자원을 꼭 삭제해줘야한다.**
...
이제 실제 애플리케이션을 배포하고, 부하를 주어 오토스케일링을 테스트 해보자.
...
상태값을 제거하고 Metric을 정리해주는 툴인 `neat` 플러그인을 설치한다.
...
이제 부하를 주어 테스트해보자. 파드가 증가하는 모습은 아래의 사진을 통해 자세하게 확인할 수 있다!
...
**전 주차에 작업했던 그라파드를 통해 현재 테스트를 대시보드로 확인할 수 있다.**
...
HPA는 동일한 스펙의 파드의 개수를 늘리는 것이었다면, VPA는 현재 실행중인 파드의 리소스를 확장한다. 이 과정에서 재시작이 필요하다는 단점이 있다. 파드를 재시작해야되기에, 실제 운영환경에서는 별로 추천하지 않는다고 한다. 

하지만, 스터디원분이 알려주신 아래의 문서와 같이 재실행없이 리소스 변경이 가능하다면 운영환경에서도 유용하게 쓰일 것 같다.
...
자세한 내용은 악분님이 잘 작성해주셨다. [악분님 포스트](https://malwareanalysis.tistory.com/603)
...
이제 부하 테스트를 위해, 위에서도 사용했던 php-apache 배포한다.
...
특정시간(0,15,30,45)에 시작하고 (05,20,35,50)종료하는 이벤트 기반 정책 생성해보자.
아래의 trigger부분을 잘 확인하면 이벤트 기반 정책을 확인할 수 있다.
...
이제 리소스를 확인해보자 현재 시각이 45분이기에 ACTIVE 상태임을 확인할 수 있다.
```bash
# 확인! 
...
50분이 되었으니 다시 확인해본다, 아래의 ACTIVE 속성이 'False'로 바뀐 것을 볼 수 있다.
```bash
$date
...
명령어를 통해 확인하면 다른 정보들도 많아 불편하다. 역시 이럴 때는 시각화도구를 통해 확인하면 한눈에 확인이 가능하다. 아래는 그라파나 대시보드 사진이다.

![](https://velog.velcdn.com/images/han-0315/post/31fb45aa-fb3a-4c93-b9d2-c4a5eb4fbbbf/image.png)


## CA - Cluster Autoscaler(CAS)
파드가 아닌 클러스터 오토스케일링을 위한 오토스케일러이다. CA는 노드의 리소스를 파악하고, 리소스 메트릭에 의해 트리거되어 노드를 스케일링한다. 오토스케일러 동작을 위해 별도의 디플로이를 배포해둔다. EKS에서는 ASG 그룹을 활용하여 노드를 스케일링한다. 

**CA는 `pending` 상태의 파드가 존재할 경우, 워커노드를 스케일 아웃한다.**
...
일부로 워커노드가 감당못하게 request를 500m = 0.5 코어로 할당한 뒤 스케일링을 통해 15개로 늘렸다.
이제 부하 테스트를 진행한다. CA의 방식은 Pending되는 파드에 의해 스케일링 되므로 파드 하나당 리소스 요청량을 크게 잡고, 많은 파드를 배포하여 스케일링을 확인한다.
...
```
노드가 추가적으로 붙는 모습을 확인할 수 있다. 관련된 eks-node-view, 그라파나 사진도 아래에 있다.
```bash
$kubectl get nodes
...
데몬셋도 적고, 여러 가지 노드의 기본적인 세팅이 별로 없어서 테스트 결과 빠르지만, ASG를 이용하기에 실제 운영환경에서는 빠르지 않다. 자동으로 축소하는 10분정도로 축소도 느리다.

## CPA - Cluster Proportional Autoscaler
CPA는 노드 비례 파드 스케일링이다. 즉, 노드 개수에 비례하여 파드를 스케일링한다.
coredns 파드와 같이 클러스터가 커질수록 부하가 증가하는 **중요한 파드는 노드의 개수에 비례해서 파드를 오토스케일링**한다. 직접 규칙을 만들어서 정책을 수립한다. **Metrics server 등을 사용하지 않고 kubapi server API를 사용**한다. 


**[cluster-proportional-autoscaler 배포]**
```bash
...
이제 노드를 증가시켜 테스트를 진행한다. 우리가 작성한 규칙은 아래와 같다. 아래는 각 [노드, 파드] 형식이다. 
즉 4번째를 예시로 들면 노드의 개수가 4개이면 파드는 3개여야 한다는 뜻이다. 나머지는 정비례하지만, 4개일때만 파드가 3개로 정비례하지 않는다.
`[1, 1], [2, 2], [3, 3], [4, 3], [5, 5]`
...
# 죽지 않고, 스케줄불가 상태로 두면 알아서, [4,3] 규칙 만족
$k get no
NAME                                               STATUS                     ROLES    AGE    VERSION
...
이제 Karpenter 실습 환경 준비를 위해서 현재 EKS 실습 환경 전부 삭제
...
Karpenter는 오픈소스 노드 수명주기 관리 솔루션으로, 기존 오토스케일링 툴과는 다르게 초단위로 컴퓨팅 리소스를 제공한다. 아래의 그림과 같이 기존의 오토스케일링에서 거치던 단계를 생략하고 바로 인스턴스를 띄우기에 초단위가 가능하다. 자세한 내용은 스터디원분의 블로그를 참고하면 된다.
...
**[이제 Karpenter를 배포한다.]**
Karpernter는 EKS에 설치하기 위해서는 spot 인스턴스에 대한 권한을 가진 IAM Role을 생성해야 한다.
...
이제 아래에서는 리소스를 프로비저닝할 때에 대한 설정을 진행한다.
간단하게 내용을 요약하면 아래와 같다.
1. spot 인스턴스로 노드를 생성한다.
2. 프로비저닝 후  `securityGroupSelector`와 `subnetSelector`를 사용하여 찾는다.
3. ttlSecondsAfterEmpty 시간을 기다리고 미사용 노드를 정리한다. 다만 데몬셋은 제외된다. 
...
파드 한 개당 요구량을 1코어로 두고, 이후 파드를 증가시켜 오토스케일링을 확인한다.
...
```
이제 Karpernter의 로그를 확인하면, Pending 상태의 파드를 확인하고 노드를 생성하는 것을 확인할 수 있다.
...
오토스케일링된 스팟 인스턴스를 AWS CLI를 통해 확인해보자.
...
이제 인스턴스 타입을 추가한 새로운 Provisiner 설치해보자.
Requirement에 `node.kubernetes.io/instance-type`가 추가된 것을 확인할 수 있다.
...
로그로 시간을 확인해보면 10초 이내로 반응하는 것을 볼 수 있다.
...
고사양의 인스턴스이다보니 과금이 무서워서, 빠르게 테스트하느라 kube-ops-view 화면 등 시각화 대시보드를 포스팅하지 못했지만, 실시간으로 노드가 생성되고 삭제되는 것을 확인할 수 있었고 신기하고 재밌었다. 
...
## 마무리
이번 포스팅에서는 EKS, K8S에서 자주 사용하는 오토스케일링 방법에 대해 알아보았다. 주제가 오토스케일링이다보니 실습이 종료되면 빠르게 리소스를 정리하는 것을 잊지 말아야 한다! 지금 한번 더 확인하길 추천한다. 필자도 CloudFormation Stack 제거를 한 후 확인하지 않았는데, Stack 제거 과정에서 오류가 발생하면서 모든 리소스가 정리되지 않았던 사고가 있었다.
