...
3. **서비스 어카운트를 지정하여 파드 생성 후 권한 테스트**
...
![](https://velog.velcdn.com/images/han-0315/post/e283534b-1249-4de9-8aae-8abf9aa857a0/image.png)
...
위의 정보를 `aws-auth` 를 보낸다. `aws-auth` 는 쿠**버네티스 내의 User, Group으로 맵핑하는 개체로 맵핑된 쿠버네티스 개체를 반환한다.** 쿠버네티스 개체를 통해 RBAC 인가 단계를 거친 후 etcd에 접근한다.
...
> 이제 쿠버네티스 **RBAC 인가**를 처리합니다. 개인적인 생각이지만 플랫폼간 인증 이외에 인가까지 처리 통합은 쉽지 않은 것 같습니다
> 
